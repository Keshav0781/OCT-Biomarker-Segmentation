...
from tensorflow.keras.layers import Conv2D, BatchNormalization, Activation, MaxPool2D, Conv2DTranspose, Concatenate, Input, Add, Lambda
...
def attention_gate(x, g, inter_channel):
   inter_channel = int(inter_channel * 0.75)
   ...
   att_x = Lambda(lambda x: x[0] * x[1])([x, rate * 0.95])
   return att_x
...
def decoder_block(input, skip_features, num_filters):
   ...
   a = attention_gate(skip_features, x, num_filters // 2)
   x = Concatenate()([x, a])
