...
# Results folder
results_folder = os.path.join(base_path, "results", "DeepResunet_aug")
...
def elastic_transform(image, mask, alpha=500, sigma=20, random_state=None):
    ...
    shape = image.shape[:2]  # Only consider height and width
    ...
    distorted_mask = np.expand_dims(distorted_mask, axis=-1)  # Add channel dimension back
...
def apply_medical_augmentation(image, mask):
    ...
    # Simulate different tissue properties (gamma correction)
